...
exports.TABLE = exports.LEGACY_TABLE = void 0;
...
exports.LEGACY_TABLE = LEGACY_TABLE;
...
async function markReady(cfg) {
    await withClient(cfg.databaseUrl, async (c) => {
        await ensureTable(c);
        await c.query(`INSERT INTO ${TABLE} (version) VALUES ($1)
       ON CONFLICT (version) DO UPDATE SET marked_at = now()`, [cfg.version]);
    });
}
async function waitForReady(opts) {
    ...
    await withClient(opts.databaseUrl, async (c) => {
        while (true) {
            ...
            try {
                const r = await c.query(`SELECT EXISTS (SELECT 1 FROM ${TABLE} WHERE version = $1) AS exists`, [opts.version]);
            ...
            if (opts.onPoll) {
                try {
                    const latest = await c.query(`SELECT version FROM ${TABLE} ORDER BY marked_at DESC LIMIT 1`);
